...
if (mysqli_num_rows($conn->query($presql)) == 1){
	echo "Successfull insertion of the employee's IRS number";
	
	$sql = $conn->query("SELECT First_Name, Last_Name, IRS_Num, Social_Sec_Num FROM EMPLOYEE WHERE IRS_Num = $irs");
	 echo '<form action="delete_employee1.php" method="post">
...
while ($row = mysqli_fetch_assoc($sql)) {
  ...
  Click Proceed to complete the deletion of the employee<br><br>
  <input type="submit" value="Submit">
  </form>';
}
else{ 
	echo 'no employee with this IRS number found. Press try again to reeenter data';
}
}
?>
<br><br>
